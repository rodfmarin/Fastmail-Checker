...
function getFastmailUrl() {
  return "https://fastmail.com/mail/";
}
...
function getFeedUrl() {
  // "zx" is a Fastmail query parameter that is expected to contain a random
  // string and may be ignored/stripped.
  return getFastmailUrl() //+ "feed/atom?zx=" + encodeURIComponent(getInstanceId());
}

function isFastmailUrl(url) {
  // Return whether the URL starts with the Fastmail prefix.
  return url.startsWith(getFastmailUrl());
}

// A "loading" animation displayed while we wait for the first response from
// Fastmail. This animates the badge text with a dot that cycles from left to
// right.
...
function updateIcon() {
  if (!localStorage.hasOwnProperty('unreadCount')) {
    chrome.browserAction.setIcon({path:"fastmail_not_logged_in.png"});
    chrome.browserAction.setBadgeBackgroundColor({color:[190, 190, 190, 230]});
    chrome.browserAction.setBadgeText({text:"?"});
  } else {
    chrome.browserAction.setIcon({path: "fastmail_logged_in.png"});
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.method == "getUnreadCount")
...
//         //var fullCountSet = xmlDoc.evaluate("/Fastmail:feed/Fastmail:fullcount",xmlDoc, FastmailNSResolver, XPathResult.ANY_TYPE, null);
//         //var fullCountNode = fullCountSet.iterateNext();
//         if (fullCountNode) {
//           handleSuccess(fullCountNode.textContent);
//           return;
//         } else {
//           console.error(chrome.i18n.getMessage("Fastmailcheck_node_error"));
//         }
//       }
...
//     xhr.send(null);
//   } catch(e) {
//     console.error(chrome.i18n.getMessage("Fastmailcheck_exception", e));
//     handleError();
//   }
}

function FastmailNSResolver(prefix) {
  if(prefix == 'Fastmail') {
...
function goToInbox() {
  ...
  chrome.tabs.getAllInWindow(undefined, function(tabs) {
    for (var i = 0, tab; tab = tabs[i]; i++) {
      if (tab.url && isFastmailUrl(tab.url)) {
        console.log('Found Fastmail tab: ' + tab.url + '. ' +
    ...
    console.log('Could not find Fastmail tab. Creating one...');
    chrome.tabs.create({url: getFastmailUrl()});
  });
}
...
var filters = {
  ...
  url: [{urlContains: getFastmailUrl().replace(/^https?\:\/\//, '')}]
};

function onNavigate(details) {
  if (details.url && isFastmailUrl(details.url)) {
    console.log('Recognized Fastmail navigation to: ' + details.url + '.' +
                'Refreshing count...');
    startRequest({scheduleRequest:false, showLoadingAnimation:false});
  }
}
